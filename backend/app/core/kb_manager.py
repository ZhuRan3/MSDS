...
from app.core.msds_llm_client import call_llm
...
class KnowledgeBaseManager:
    ...
    def _save(self):
        """保存知识库"""
        self.kb_file.parent.mkdir(exist_ok=True)
        with open(self.kb_file, 'w', encoding='utf-8') as f:
            json.dump(self.data, f, ensure_ascii=False, indent=2)

    def add(self, cas: str, name_cn: str = "", use_llm: bool = True) -> bool:
        """添加化学品到知识库"""
        logger.info(f"添加化学品: {cas} {'(' + name_cn + ')' if name_cn else ''}")

        # 如果已存在，先移除
        if cas in self.data:
            logger.info(f"{cas} 已存在，将更新")
            del self.data[cas]
        ...
        if not pubchem_data:
            logger.warning(f"无法从PubChem获取数据: {cas}")
            return False

        logger.info(f"CID: {pubchem_data['cid']}")
        ...
        logger.info(f"已保存到知识库: {cas}")
        logger.info(f"  化学家族: {chem.get('chemical_family', 'N/A')}")
        logger.info(f"  UN编号: {chem.get('un_number', 'N/A')}")
    ...
    def remove(self, cas: str) -> bool:
        ...
        if cas in self.data:
            del self.data[cas]
            self._save()
            logger.info(f"已移除: {cas}")
            return True
        logger.info(f"未找到: {cas}")
        return False
