...
def get_active_provider() -> Optional[str]:
    ...
    if settings.ANTHROPIC_API_KEY:
        return "anthropic"
    if settings.HUANTONG_LLM_API_KEY:
        return "openai"
    if settings.HUANTONG_LLM_PROVIDER == "ollama":
        return "ollama"
    return None
...
def _call_anthropic(prompt: str, system_prompt: str, temperature: float, max_tokens: int) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(f"Anthropic API调用失败: {e}")
        return None
...
def _call_openai(prompt: str, system_prompt: str, temperature: float, max_tokens: int) -> Optional[str]:
    ...
    try:
        import requests
        headers = {
            "Authorization": f"Bearer {settings.HUANTONG_LLM_API_KEY}",
            "Content-Type": "application/json"
        }
        data = {
            "model": settings.HUANTONG_LLM_MODEL,
            "messages": [
        ...
        resp = requests.post(f"{settings.HUANTONG_LLM_BASE_URL}/chat/completions", headers=headers, json=data, timeout=60)
...
def _call_ollama(prompt: str, system_prompt: str, temperature: float) -> Optional[str]:
    ...
    try:
        import requests
        data = {
            "model": settings.HUANTONG_LLM_MODEL or "llama3",
            "prompt": prompt,
            "system": system_prompt,
            "temperature": temperature,
            "stream": False
        }
        resp = requests.post(f"{settings.HUANTONG_LLM_BASE_URL}/api/generate", json=data, timeout=120)
        resp.raise_for_status()
