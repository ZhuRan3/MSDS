        ...
        for cls_str in component.ghs_classifications:
            if target_prefix in cls_str:
                try:
                    ...
                    table = ACUTE_TOXITY_CONVERSION.get("inhalation_vapor" if route == "inhalation" else route, {})
                    if cat_num in table:
        ...
        for threshold, cat in thresholds:
            if ate <= threshold:
                table = ACUTE_TOXITY_CONVERSION.get(
                    "inhalation_vapor" if route == "inhalation" else route, {}
                )
                info = table.get(cat, {})
                return {
...
def load_component_from_kb(cas: str, name: str = "") -> Optional[Dict]:
    ...
    if isinstance(kb, dict):
        ...
        for key, entry in kb.items():
            if name and (name in entry.get("chemical_name_cn", "") or name in entry.get("chemical_name_en", "")):
                return entry
