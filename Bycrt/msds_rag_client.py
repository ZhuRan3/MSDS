...
KB_DIR = Path("D:/Learning/Jilema/MSDS-main/Bycrt/reference_db")
...
class ChemicalKnowledgeBase:
    ...
    def _load_knowledge_base(self):
        """加载知识库"""
        # 加载扩展知识库
        db_file = KB_DIR / "chemical_reference_db_extended.json"
        if db_file.exists():
        ...
        # 加载合并数据库
        merged_file = KB_DIR / "merged_chemical_db.json"
        if merged_file.exists():
            with open(merged_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                for cas, chem in data.items():
                    if cas not in self.chemicals:
                        self.chemicals[cas] = chem
                        self._index_chemical(cas, chem)

        print(f"  [OK] 知识库已加载: {len(self.chemicals)} 种化学品")
        ...
        for name in names:
            ...
            for kw in keywords:
                if kw not in self.name_index:
                    self.name_index[kw] = []
                self.name_index[kw].append((cas, len(kw)))  # 关键词越长分数越高
    ...
    def retrieve(self, query: str, top_k: int = 3) -> List[Dict]:
        ...
        for cas in self.chemicals:
            if cas.replace('-', '') == query_clean:
                results.append((cas, 100))  # 精确匹配最高分
...
if __name__ == "__main__":
    retriever = ChemicalRAGRetriever()

    # 测试检索
    test_queries = ["106-44-5", "acetone", "丙酮", "醇类"]

    for q in test_queries:
        print(f"\n{'='*50}")
        print(f"检索查询: {q}")
        result = retriever.retrieve_for_msds(q)
        print(f"知识库命中: {result['has_knowledge']}")
        if result['has_knowledge']:
            primary = result['primary_chemical']
            print(f"主匹配: {primary.get('chemical_name_cn', 'N/A')} (CAS: {primary.get('cas_number', 'N/A')})")
            print(f"相关度: {primary.get('_relevance_score', 0)}%")